...
/**
 * IdemixSignature is a class class for Signatures.
 */
public class IdemixSignature {
     ...
     */
    public IdemixSignature(final IdemixCredential c, final BIG sk, final IdemixPseudonym pseudonym,
        final IdemixIssuerPublicKey ipk, final boolean[] disclosure, final byte[] msg, final int rhIndex, final Idemix.CredentialRevocationInformation cri) {
        if (c == null || sk == null || pseudonym == null || pseudonym.getNym() == null || pseudonym.getRandNym() == null
            || ipk == null || disclosure == null || msg == null || cri == null) {
     ...
     */
    public IdemixSignature(final Idemix.Signature proto) {
        if (proto == null) {
            throw new IllegalArgumentException("Cannot construct idemix signature from null input");
        }
     ...
     */
    public boolean verify(final boolean[] disclosure, final IdemixIssuerPublicKey ipk, final byte[] msg,
        final BIG[] attributeValues, final int rhIndex, final PublicKey revPk, final int epoch) throws CryptoException {
        if (disclosure == null || ipk == null || msg == null || attributeValues == null
            || attributeValues.length != ipk.getAttributeNames().length || disclosure.length != ipk.getAttributeNames().length) {
            return false;
        }
        ...
        byte[] nonRevokedProofBytes = nonRevokedVerifier.recomputeFSContribution(this.nonRevocationProof,
         proofC, IdemixUtils.transformFromProto(this.revocationPk), proofSRh);
     ...
     */
    private int[] hiddenIndices(final boolean[] disclosure) {
        if (disclosure == null) {
